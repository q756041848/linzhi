...
class Tp extends Taglib
{
    ...
    public function tagMenu($tag)
    {
        ...
        foreach ($menu as $k => $m) {
            $m = strtolower($m);
            $url = isset($urlArr[$k]) && $urlArr[$k] ? $urlArr[$k] : (substr($m, 0, 1) == 's' || substr($m, 0, 1) == 'm' || substr($m, 0, 1) == 'n' ? substr($m, 1) : $m);
            $urls = explode(":", $url);
            ...
            switch ($m) {
                ...
                case 'forbid':
                    ...
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= '<a href="javascri pt:;" onclick="forbid_all(\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" class="btn btn-warning radius mr-5"><i class="Hui-iconfont">&#xe631;</i> ' . $title . '</a>';
                    break;
                case 'resume':
                ...
                case 'mdelete':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '删除';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="del(this,\'{$sec.id}\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>';
                    break;
                case 'ndelete':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '删除';
                    list($url, $param) = $this->parseUrl($url);
                    $parseStr .= ' <a title="' . $title . '" href="javascript:;" onclick="del(this,\'{$thr.id}\',\'<?php echo \think\Url::build(\'' . $url . '\', [' . $param . ']); ?>\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>';
                    break;
                case 'srecycle':
                    $title = isset($titleArr[$k]) && $titleArr[$k] ? $titleArr[$k] : '还原';
                    list($url, $param) = $this->parseUrl($url);
